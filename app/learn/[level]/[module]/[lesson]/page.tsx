...
export default async function LessonPage({ params }: { params: Promise<{ level: string; module: string; lesson: string }> }) {
  ...
  if (!item) return <main className="shell page-shell"><h1>Lesson not found</h1></main>;
  const { previous, next } = getAdjacentLessons(item);
  return <main className="shell lesson-page"><LessonWorkspace lesson={item} previous={previous} next={next} /></main>;
}
